...
/*									    */
/*         --> Heuristik auf der Basis einer Vorrangregel/Variation         */
/*									    */
/* ************************************************************************ */
...
static int LowerBoundCalc_J(job_nr, num_of_pred, local_head, op)
  ...
  */
  while ( cut != NIL ) 
  {
     ...
     */
     ...
     if ( genpos > 2 ) 
     {
	low_for_prior_op_j = LowerBoundCalc_J(minreadytimejob, num_of_pred, 
						local_head, prior_op_j);
	for ( i = 2; i < genpos; ++i )
	{
	   low_for_op = LowerBoundCalc_J(minreadytimejob, num_of_pred,
					       local_head, generate_set[i]);

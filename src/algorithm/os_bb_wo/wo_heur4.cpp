...
/*             HIER: BOTTLENECK-MATCHINGPROBLEM / MAXIMIERUNG                */ 
/* ************************************************************************* */
...
/* loest Bottleneck-Zuordnungsproblem:
   ...
   das minimale Kantengewicht maximal wird;
*/
   ...
   */
   while ( iter < NumOfOperations )
   {
      ...
      */
      ...
      {
	 /* hier: (vgl.heur3.c: pos. Gewichte)
	    Kantengewichte = neg. Bearbeitungszeiten, falls num_of_pred = 0,
	 */
	 if ( num_of_pred[sum] == 0 )
	 {
	    A[i].s = OpData[sum].job_nr;
	    A[i].t = OpData[sum].machine_nr + NumOfJobs;
	    A[i].weight = -OpData[sum].process_time;
	    sum++;
	 }
	 else
	 {

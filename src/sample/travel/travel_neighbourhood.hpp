...
/** @name Neighbourhoods for the Traveling Salesman Problem\\
    \\
    \\
    \\
    ...
    \\
    \\
    The #API# neighbourhood swaps citys at two adjacent positions to generate a neighbour.
    It supports both enumerative generation of neighbours and anti neighbours.\\
    \\
    For example #A = { 1 0 2 3 4 }# is a neighbour of #B = { 0 1 2 3 4 }#.\\
    \\
    Therefore in enumeration mode a solution for a problem with #n# citys has #n# neighbours.
    An anti neighbour is generated by using the #RPI# neighbourhood.\\
    \\
*/
//@{
...
private:
  ...
  // our problem .. can provide us with an initial solution
  // and is needet to calculate the objective
  Travel_Graph* prob;
...
protected:
  ...
  /** create and save a tabu representation for a solution ...\\
      ...
      As one can easily see the solutions #A = { 0 1 2 3 4 }# and #B = { 2 3 4 0 1 }#
      are the same since a solution is always a complete cycle. Our traveling 
      salesman problem is symmetrical, so #C = { 3 2 1 0 4 }# is also equal to #A# 
      and #B#.

      Additionally all neighbours of #A# are equal to all neighbours of #B# and #C#. We 
      now have to make sure that if #A# is on the tabu list also #B# and #C# are on the 
      tabu list. Therefore we will create a tabu representation of a solution that
      is equal for #A#, #B# and #C#.

      To accomplish that we will start reading from the #0# in each solution and read 
      into the direction where the smaller city is. So, #A#,#B# and #C# have the 
      tabu representation #D = { 0 1 2 3 }#.
  */

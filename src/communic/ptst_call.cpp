																		...
																		TCL_HACK_CHAR *argv[]) 
{
  ...
  str = G_Preferences.CONFIG_HOME+"/proc/" + DEFAULT_IN_FILE;
...
int start_paratest(Tcl_Interp *interp, 
		...
		if (str2=="-1") {
				G_ExceptionList.lthrow("no file: "+G_Preferences.LISA_HOME+"/bin/"+algo_call+ " in LiSA path",Lisa_ExceptionList::END_OF_FILE);
				return !OK;
		}

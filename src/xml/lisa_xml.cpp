		...
		if (!kl.empty())
				{  
						...
						do {
								...
								if(!conv_name)
										{
												cerr << "Warning : Failed to recode " << endl;
		...
		for(cur = cur->children; cur; cur = cur->next)
				{
						...
						if(!xmlStrEqual(cur->name, (const xmlChar*) "parameter"))
								continue;
						//get type
						c_type = xmlGetProp(cur, (const xmlChar*) "type");
						para_type = "";
						if(c_type)
								{
										if(!xmlChar2string(c_type,para_type))
												cerr << "Warning : Failed to recode " << endl;
										xmlFree(c_type);
								}
						...
						if(c_name)
								{
										if(!xmlChar2string(c_name,para_name))
												cerr << "Warning : Failed to recode " << endl;
										xmlFree(c_name);
								}
						c_value = xmlGetProp(cur, (const xmlChar*) "value");
						para_value = "";
						if(c_value)
								{
										if(!xmlChar2string(c_value,para_value))
												cerr << "Warning : Failed to recode " << endl;
										xmlFree(c_value);
								}
						pipe.clear();
...
xmlChar* string2xmlChar(const std::string& input){
		...
		if (isolat1ToUTF8(out, &out_size, (const unsigned char*) input.c_str(), &temp) || temp-size) {
				xmlFree(out);
				//cout << "FAILED" << endl;
				return NULL;
		}
